...
const tutorialSteps = [
  {
    target: null,
    title: "Bienvenue sur DigitalStore ! 🎉",
    text: "Ce tutoriel vous guide en 6 étapes pour utiliser toutes les fonctionnalités de votre boutique.",
    position: "center"
  },
  {
    target: ".nav-links .nav-link:nth-child(2)",
    title: "Le Catalogue 🛍️",
    text: "Cliquez sur 'Catalogue' pour explorer nos 224 produits. Filtrez par catégorie, prix ou note.",
    position: "bottom"
  },
  {
    target: "#searchIn",
    title: "Recherche rapide 🔍",
    text: "Tapez le nom d'un produit ou d'une marque pour le trouver instantanément.",
    position: "bottom",
    action: () => { nav('catalog'); }
  },
  {
    target: ".cart-btn",
    title: "Votre panier 🛒",
    text: "Ajoutez des produits et cliquez ici pour voir votre panier. Vous pouvez entrer un code promo : DIGITAL10",
    position: "bottom"
  },
  {
    target: "#connectBtn",
    title: "Créez votre compte 👤",
    text: "Connectez-vous pour sauvegarder vos commandes et votre wishlist. C'est gratuit et rapide !",
    position: "bottom"
  },
  {
    target: ".nav-link:last-of-type",
    title: "Suivi de commande 📦",
    text: "Après votre achat, suivez votre livraison en temps réel avec votre numéro de commande.",
    position: "bottom"
  }
];
...
function showTutStep() {
  ...
  if (tutHighlighted) { tutHighlighted.classList.remove('tut-highlight'); tutHighlighted = null; }

  // Execute action if any
  if (step.action) step.action();

  // Content
  document.getElementById('tutorialStep').textContent = `Étape ${tutStep + 1} / ${tutorialSteps.length}`;
  document.getElementById('tutorialTitle').textContent = step.title;
  document.getElementById('tutorialText').textContent = step.text;
  document.getElementById('tutorialProgress').textContent = `${tutStep + 1}/${tutorialSteps.length}`;
  document.getElementById('tutPrev').disabled = tutStep === 0;
  document.getElementById('tutNext').textContent = tutStep === tutorialSteps.length - 1 ? '✓ Terminer' : 'Suivant →';
  ...
  if (!step.target || step.position === 'center') {
    ...
    if (el) {
      ...
      if (step.position === 'bottom') {
        box.style.top = (r.bottom + 20) + 'px';
        box.style.left = Math.min(r.left, window.innerWidth - 340) + 'px';
        arrow.style.top = '-8px'; arrow.style.left = '30px';
        arrow.style.bottom = ''; arrow.style.right = '';
      } else {
        box.style.top = (r.top - 10 - box.offsetHeight) + 'px';
        box.style.left = Math.min(r.left, window.innerWidth - 340) + 'px';
        arrow.style.bottom = '-8px'; arrow.style.left = '30px';
        arrow.style.top = ''; arrow.style.right = '';
      }
    }
  }
}
...
function tutorialNext() {
  if (tutStep < tutorialSteps.length - 1) { tutStep++; showTutStep(); }
  else closeTutorial();
}
function tutorialPrev() { if (tutStep > 0) { tutStep--; showTutStep(); } }
